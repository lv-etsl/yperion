...
function be_display_posts_shortcode( $atts ) {
	...
	$wrapper_options = array( 'ul', 'ol', 'div', 'article', 'section' );
	if ( ! in_array( $wrapper, $wrapper_options, true ) ) {
		$wrapper = 'section';
	}
	$inner_wrapper = 'div' === $wrapper ? 'div' : 'article';
	...
	while ( $dps_listing->have_posts() ) :
		...
		if ( $image_size && has_post_thumbnail() && $include_link ) {
			$image = '<a class="img-fluid" href="' . get_permalink() . '">' . get_the_post_thumbnail( get_the_ID(), $image_size ) . '</a> ';

		} elseif ( $image_size && has_post_thumbnail() ) {
			$image = '<span class="img-fluid">' . get_the_post_thumbnail( get_the_ID(), $image_size ) . '</span> ';
